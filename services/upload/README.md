...
4. **Очередь транскодера (Redpanda/Kafka)**
   ...
   - Сообщение содержит `track_id`, `artist_ids[]` и `track_url`. Транскодер скачивает оригинал по ссылке, обрабатывает его и уже после этого вызывает Track Service для обновления `storage_url` и других полей.
