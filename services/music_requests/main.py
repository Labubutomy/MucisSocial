...
async def update_user_coins(
    user_id: UUID, coins_delta: int, authorization: str
) -> None:
...
async def add_track_to_queue(
    artist_id: UUID, track_id: UUID, authorization: str
) -> None:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            response.raise_for_status()
            logger.info(
                f"Successfully added track {track_id} to artist {artist_id} queue"
            )
        except httpx.HTTPError as e:
...
@app.post(
    "/api/v1/sessions",
    response_model=RequestSessionOut,
...
async def get_my_active_session(
    ...
    result = await db.execute(
        ...
        .where(
            and_(RequestSession.artist_id == user_id, RequestSession.is_active == True)
        )
...
@app.post(
    "/api/v1/requests",
    response_model=TrackRequestOut,
    status_code=status.HTTP_201_CREATED,
)
async def create_track_request(
    ...
    # Get session
    result = await db.execute(
        select(RequestSession).where(
            RequestSession.session_code == request_in.session_code
        )
    )
    session = result.scalar_one_or_none()
...
async def get_incoming_requests(
    ...
    status_filter: str | None = Query(
        None, description="Filter by status: pending, accepted, declined"
    ),
):
...
async def handle_track_request(
    ...
    # Get request
    result = await db.execute(select(TrackRequest).where(TrackRequest.id == request_id))
    ...
    if track_request.artist_id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not your request to handle"
        )
    ...
    if action.action == "accept":
        ...
        await update_user_coins(
            track_request.requester_id, settings.coin_cost_per_request, authorization
        )
